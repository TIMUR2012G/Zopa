document.addEventListener('DOMContentLoaded', function () {
    ...
    function verifyCaptcha() {
        ...
        if (inputText === captchaText.toLowerCase()) {
            captchaStatus.textContent = 'КАПЧА ЕБНУЛАСЬ!';
            captchaStatus.style.color = 'green';
            document.getElementById("exit").classList.add("unlock");
        } else if (inputText.length < 6) {
            captchaStatus.textContent = 'НАПИШИ БЛ*ТЬ ВСЕ СИМВОЛЫ';
            captchaStatus.style.color = 'red';
        } else {
            captchaStatus.textContent = 'НЭ ПРАВИЛЬНО';
            captchaStatus.style.color = 'red';
        }
        setTimeout(() => {
            captchaStatus.textContent = 'статус: АХ*ЕННЫЙ';
